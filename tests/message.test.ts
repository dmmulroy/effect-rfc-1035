	...
	it.effect("successfully decodes specific test cases", () =>
		Effect.gen(function* () {
			...
			if (Exit.isSuccess(result)) {
				...
				expect(message.question).toHaveLength(1);
				expect(message.question[0]!.qname.labels).toHaveLength(2);
				expect(Array.from(message.question[0]!.qname.labels[0]!)).toEqual([
					101,
					120,
					97,
					109,
				...
				expect(Array.from(message.question[0]!.qname.labels[1]!)).toEqual([
					99,
					111,
					109, // "com"
				]);
				expect(message.question[0]!.qtype).toBe(RRTypeNameToRRType.A);
				expect(message.question[0]!.qclass).toBe(1);
			}
		}),
	);
	...
	it.effect("successfully decodes localhost queries", () =>
		Effect.gen(function* () {
			...
			if (Exit.isSuccess(result)) {
				...
				expect(message.question[0]!.qname.labels).toHaveLength(1);
				expect(Array.from(message.question[0]!.qname.labels[0]!)).toEqual([
					108,
					111,
					99,
	...
	it.effect("handles various QTYPE values correctly", () =>
		Effect.gen(function* () {
			...
			for (const { qtype, bytes } of qtypeTests) {
				...
				if (Exit.isSuccess(result)) {
					expect(result.value.question[0]!.qtype).toBe(qtype);
				}
			}
		}),
	);
	...
	it.effect("handles maximum length domain names", () =>
		Effect.gen(function* () {
			...
			if (Exit.isSuccess(result)) {
				...
				expect(decodedMessage.question[0]!.qname.labels).toHaveLength(4);
				expect(decodedMessage.question[0]!.qname.labels[0]!.length).toBe(63);
				expect(decodedMessage.question[0]!.qname.labels[1]!.length).toBe(63);
				expect(decodedMessage.question[0]!.qname.labels[2]!.length).toBe(63);
				expect(decodedMessage.question[0]!.qname.labels[3]!.length).toBe(4);
			}
		}),
	);
