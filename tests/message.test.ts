...
describe("message", () => {
	it.effect.prop(
		...
		[arbitraryValidDnsMessageUint8Array],
		([{ messageBuffer, header, questions }]) =>
			Effect.gen(function* () {
				...
				if (Exit.isSuccess(result)) {
					...
					expect(message.questions).toHaveLength(questions.length);
					expect(message.questions).toHaveLength(header.qdcount);

					for (let qIdx = 0; qIdx < questions.length; qIdx++) {
						const expectedQuestion = questions[qIdx]!;
						const actualQuestion = message.questions[qIdx]!;
		...
		([{ messageBuffer, header, questions }]) =>
			Effect.gen(function* () {
				...
				if (Exit.isSuccess(result)) {
					...
					// Validate questions match expected patterns
					// expect(message.questions).toHaveLength(questions.length);
					// expect(message.questions).toHaveLength(1); // Common messages have 1 question

					// const question = questions[0]!;
					// const actualQuestion = message.questions[0]!;
